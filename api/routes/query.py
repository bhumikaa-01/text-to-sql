...
async def approve_endpoint(
    ...
    """
    Execute a previously flagged SQL statement
    if the user approves it.

    Always re-runs the HITL check before execution
    ...
    if guard[
        ...
        logger.warning(
            "Executing approved write SQL: %s",
            request.sql[:200],
        )
    ...
    try:
        ...
        logger.error(
            "Approve endpoint execution error: %s",
            exc,
            exc_info=True,
        )
